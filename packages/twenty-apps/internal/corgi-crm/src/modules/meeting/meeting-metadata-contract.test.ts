...
describe('Meeting booking metadata', () => {
  ...
  it('links every booking to its RIA, owner, and app-written booking actor', () => {
    ...
    expect(bookedByOnMeetingBooking.config).toMatchObject({
      name: 'bookedBy',
      // writability is immutable on an existing relation field, so this pins
      // what the server will actually accept, not what we would prefer.
      isUIEditable: false,
      writability: MetadataWritability.APPLICATION,
      relationTargetObjectMetadataUniversalIdentifier:
